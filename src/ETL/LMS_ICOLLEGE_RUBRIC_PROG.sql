...
SELECT
  ...
  (
    ...
    FROM
      icollprd.D2L_RUBRIC_CRITERIA_LEVEL RubricCriteriaLevel
    WHERE
...
FROM
  icollprd.LMS_RUBRIC_PROGRAM ProgramDept,
  icollprd.D2L_RUBRIC_OBJECT RubricObject,
  icollprd.D2L_RUBRIC_ASSESSMENT RubricAssessment,
  icollprd.D2L_RUBRIC_ASSESSMENT_CRITERIA RubricAssessmentCriteria,
  (
    SELECT
      *
    FROM
      icollprd.D2L_ORGANIZATIONAL_UNIT CourseOfferingOU
    WHERE
      CourseOfferingOU.IsDeleted = 0 AND
      CourseOfferingOU.Type = 'Course Offering'
  ) CourseOffering,
  icollprd.D2L_ORGANIZATIONAL_UNIT Semester,
  icollprd.D2L_USER UserAccount,
  icollprd.D2L_RUBRIC_OBJECT_LEVELS RubricLevel,
  icollprd.D2L_RUBRIC_OBJECT_CRITERIA RubricObjectCriteria,
  icollprd.D2L_RUBRIC_OBJECT_LEVELS CriteriaLevel
WHERE
  -- RubricObject
...
USING
  (
    SELECT
      ...
      (SELECT STVCODE_DESC FROM icollprd.BNR_STVCODE WHERE STVCODE_TABLE = 'STVCOLL' AND STVCODE_CODE = SGBSTDN_COLL_CODE_1) AS COLG_DESC,
      (SELECT STVCODE_DESC FROM icollprd.BNR_STVCODE WHERE STVCODE_TABLE = 'STVDEPT' AND STVCODE_CODE = SGBSTDN_DEPT_CODE) AS DEPT_DESC,
      (SELECT STVCODE_DESC FROM icollprd.BNR_STVCODE WHERE STVCODE_TABLE = 'STVDEGC' AND STVCODE_CODE = SGBSTDN_DEGC_CODE_1) AS DEGR_DESC,
      (SELECT STVCODE_DESC FROM icollprd.BNR_STVCODE WHERE STVCODE_TABLE = 'STVMAJR' AND STVCODE_CODE = SGBSTDN_MAJR_CODE_1) AS MAJR_DESC,
      (SELECT STVCODE_DESC FROM icollprd.BNR_STVCODE WHERE STVCODE_TABLE = 'STVMAJR' AND STVCODE_CODE = SGBSTDN_MAJR_CODE_CONC_1) AS CONCENT_DESC,
      (SELECT STVCODE_DESC FROM icollprd.BNR_STVCODE WHERE STVCODE_TABLE = 'USGRGNT' AND STVCODE_CODE = STU_LEVEL_REGENTS) AS LEVEL_REGENTS_DESC,
      (SELECT STVCODE_DESC FROM icollprd.BNR_STVCODE WHERE STVCODE_TABLE = 'STVLEVL' AND STVCODE_CODE = SGBSTDN_LEVL_CODE) AS STU_LEVEL_DESC,
      (SELECT STVCODE_DESC FROM icollprd.BNR_STVCODE WHERE STVCODE_TABLE = 'STVSTYP' AND STVCODE_CODE = SGBSTDN_STYP_CODE) AS STU_TYPE_DESC,
      (SELECT STVCODE_DESC FROM icollprd.BNR_STVCODE WHERE STVCODE_TABLE = 'STVCAMP' AND STVCODE_CODE = SGBSTDN_STYP_CODE) AS PRIME_CAMP_DESC,
      GORRACE_DESC AS STU_RACE_DESC
    FROM
      icollprd.BNR_SGBSTDN,
      icollprd.BNR_GORPRAC
    WHERE
