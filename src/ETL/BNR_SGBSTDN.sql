...
FROM
  SPRIDEN@EDW_BANNER_LINK,
  SPBPERS@EDW_BANNER_LINK,
  (
    SELECT
      O.SGBSTDN_PIDM,
      ...
      BANINST1.F_PRIMARY_CAMPUS@EDW_BANNER_LINK(O.SGBSTDN_PIDM, O.SGBSTDN_TERM_CODE_EFF) AS SGBSTDN_CAMP_CODE
    FROM
      SGBSTDN@EDW_BANNER_LINK O
    WHERE
      SGBSTDN_TERM_CODE_EFF = (
        SELECT
          MAX(I.SGBSTDN_TERM_CODE_EFF)
        FROM
          SGBSTDN@EDW_BANNER_LINK I
        WHERE
